 ...
 */
export const authenticateAndGenerateStealthAddress = async (
  message: string = STATIC_MESSAGE
  ...
  try {
    ...
    if (storedCredentials.length === 0) {
      throw new Error(
        "No WebAuthn credentials found. Please run setupPassKeys() first."
      );
    }
    ...
    const stealthMetaAddress = `st:eth:0x${spendingPubKey.slice(
      2
 ...
 */
export const initializeStealthAddress =
  async (): Promise<StealthAddressResult> => {
    try {
...
export const runDemo = async (): Promise<StealthAddressResult> => {
  ...
  console.log(
    "1. Check for existing WebAuthn credentials (using excludeCredentials)"
  );
  ...
  console.log(
    "5. Generate compressed public keys using secp256k1 (33 bytes each)"
  );
